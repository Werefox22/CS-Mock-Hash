...
checkPassword = async (username, plaintextPassword) => {
    ...
    if (globalStore[username]) {
        ...
        result = await bcrypt.compare(plaintextPassword, globalStore[username])
        ...
        if (result) {
            ...
            console.log(`Failed to login to user ${username}`)
        }
    }
    else {
...
hashPassword = async (username, password) => {
    ...
    console.log(`New user '${username}' created with hash ${hash}`)
}
